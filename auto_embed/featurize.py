...
def featurize_batch(df, sequences, vocab=None, embeddings=None, ecfp_bits=2048, k=3, use_llm=False):
    x, y = [], []
...
def featurize_training_set(data_path, seqs_path, x_out_path, y_out_path, vocab_path=None,
                           k=3, ecfp_bits=2048, use_llm=False, embedding_path=None):
    df = pd.read_csv(data_path)
    sequences = load_uniprot_sequences(seqs_path)

    if use_llm:
        if embedding_path is None:
            raise ValueError("LLM embedding path must be provided when use_llm=True")
    ...
    x, y = featurize_batch(df, sequences, vocab=vocab, embeddings=embeddings,
                           ecfp_bits=ecfp_bits, k=k, use_llm=use_llm)
    ...
    return vocab if not use_llm else None
